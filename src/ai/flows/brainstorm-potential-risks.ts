...
const BrainstormPotentialRisksInputSchema = z.object({
  goalDescription: z
    .string()
    .describe(
      'A description of the goal for which to brainstorm potential risks.'
    ),
  desiredCount: z
    .number()
    .optional()
...
export type BrainstormPotentialRisksInput = z.infer<
  typeof BrainstormPotentialRisksInputSchema
>
...
const PotentialRiskObjectSchema = z.object({
  ...
  category: z
    ...
    .nullable()
    .describe(
      'Kategori risiko yang paling sesuai dari daftar yang diberikan, atau null jika tidak ada yang sangat cocok.'
    ),
})
...
const BrainstormPotentialRisksOutputSchema = z.object({
  potentialRisks: z
    ...
    .describe(
      'A list of potential risk objects, each containing a description and an associated category.'
    ),
})
export type BrainstormPotentialRisksOutput = z.infer<
  typeof BrainstormPotentialRisksOutputSchema
>
  ...
  async (input) => {
    ...
    if (output && Array.isArray(output.potentialRisks)) {
      ...
      const validatedRisks = output.potentialRisks.map((risk) => ({
        ...
        category:
          risk.category &&
          RISK_CATEGORIES.includes(risk.category as RiskCategory)
            ? risk.category
            : null,
      }))
    ...
    if (
      ...
      const adaptedRisks = (output as any).risks
        .map((r: any) => {
          if (typeof r === 'string') {
            ...
            return {
              ...
              category:
                r.category &&
                RISK_CATEGORIES.includes(r.category as RiskCategory)
                  ? r.category
                  : null,
            }
          }
        ...
        .filter((r: any) => r !== null)
      return {
        potentialRisks: adaptedRisks as Array<{
          description: string
