...
const SuggestRiskParametersInputSchema = z.object({
  ...
  riskCategory: z
    ...
    .nullable()
    .describe(
      'Kategori potensi risiko (Kebijakan, Hukum, Reputasi, Kepatuhan, Keuangan, Fraud, Operasional).'
    ),
  riskCauseDescription: z
    .string()
    .optional()
    .describe(
      'Deskripsi penyebab risiko (jika berlaku, untuk analisis penyebab).'
    ),
  goalDescription: z.string().describe('Deskripsi sasaran terkait.'),
})
export type SuggestRiskParametersInput = z.infer<
  typeof SuggestRiskParametersInputSchema
>
...
const SuggestRiskParametersOutputSchema = z.object({
  suggestedLikelihood: z
    ...
    .nullable()
    .describe(
      `Saran level kemungkinan dari daftar: ${LIKELIHOOD_LEVELS_DESC.join(
        ', '
      )}.`
    ),
  likelihoodJustification: z
    .string()
    .describe(
      'Justifikasi AI untuk saran level kemungkinan dalam Bahasa Indonesia.'
    ),
  suggestedImpact: z
    .custom<ImpactLevelDesc>()
    .nullable()
    .describe(
      `Saran level dampak dari daftar: ${IMPACT_LEVELS_DESC.join(', ')}.`
    ),
  impactJustification: z
    .string()
    .describe(
      'Justifikasi AI untuk saran level dampak dalam Bahasa Indonesia.'
    ),
})
export type SuggestRiskParametersOutput = z.infer<
  typeof SuggestRiskParametersOutputSchema
>
...
function getGuidanceText(input: SuggestRiskParametersInput): string {
  ...
  const riskTargetDescription = isCauseAnalysis
    ? input.riskCauseDescription
    : input.potentialRiskDescription
...
- Kategori Potensi Risiko Terkait: ${
    input.riskCategory || 'Tidak ditentukan'
  } (Kategori yang mungkin: ${RISK_CATEGORIES.join(', ')})
...
Level yang tersedia untuk Kemungkinan adalah: "${LIKELIHOOD_LEVELS_DESC.join(
    '", "'
  )}". Pastikan output level kemungkinan persis salah satu dari ini.
Level yang tersedia untuk Dampak adalah: "${IMPACT_LEVELS_DESC.join(
    '", "'
  )}". Pastikan output level dampak persis salah satu dari ini.
...
PANDUAN PENENTUAN DAMPAK (Gunakan kategori potensi risiko ("${
    input.riskCategory || 'Tidak ditentukan'
  }") untuk memilih area dampak yang paling relevan):
Gunakan skala dampak berikut: ${IMPACT_LEVELS_DESC[0]} (1), ${
    IMPACT_LEVELS_DESC[1]
  } (2), ${IMPACT_LEVELS_DESC[2]} (3), ${IMPACT_LEVELS_DESC[3]} (4), ${
    IMPACT_LEVELS_DESC[4]
  } (5).
...
1.  Risiko Standar Kinerja (biasanya untuk kategori Operasional, Kepatuhan, Reputasi, Hukum, Kebijakan, atau kategori umum jika tidak ada kategori spesifik lain yang lebih cocok):
    ...
    -   Penurunan Reputasi:
        -   (1) ${
          IMPACT_LEVELS_DESC[0]
        }: Jumlah pengaduan internal/eksternal ≤ 5
        ...
        -   (5) ${
          IMPACT_LEVELS_DESC[4]
        }: Pemberitaan negatif viral nasional/internasional
...
2.  Risiko Fraud (jika kategori potensi risiko adalah 'Fraud'):
    ...
    -   Fraud Kerugian Keuangan Negara (penggelapan, penyalahgunaan anggaran):
        -   (1) ${
          IMPACT_LEVELS_DESC[0]
        }: Kerugian ≤0,01% dari total anggaran non belanja Pegawai pada UPR
        -   (2) ${
          IMPACT_LEVELS_DESC[1]
        }: Kerugian >0,01% s.d 0,1% dari total anggaran non belanja Pegawai pada UPR
        -   (3) ${
          IMPACT_LEVELS_DESC[2]
        }: Kerugian >0,1% s.d 1% dari total anggaran non belanja Pegawai pada UPR
        -   (4) ${
          IMPACT_LEVELS_DESC[3]
        }: Kerugian >1% s.d 5% dari total anggaran non belanja Pegawai pada UPR
        -   (5) ${
          IMPACT_LEVELS_DESC[4]
        }: Kerugian >5% dari total anggaran non belanja Pegawai pada UPR
...
3.  Risiko Keuangan (jika kategori potensi risiko adalah 'Keuangan', bukan Fraud):
    ...
    -   (3) ${
      IMPACT_LEVELS_DESC[2]
    }: Ada temuan pengembalian uang/penyimpangan s.d 0,1% dari total anggaran
    -   (4) ${
      IMPACT_LEVELS_DESC[3]
    }: Ada temuan pengembalian uang/penyimpangan >0,1% s.d 1% dari total anggaran
    -   (5) ${
      IMPACT_LEVELS_DESC[4]
    }: Ada temuan pengembalian uang/penyimpangan >1% dari total anggaran
...
export const suggestRiskParametersFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      console.warn(
        'AI output for risk parameter suggestion was not in the expected format or was null/undefined.'
      )
      return {
        suggestedLikelihood: null,
        likelihoodJustification:
          'AI tidak dapat memberikan saran level kemungkinan berdasarkan informasi yang diberikan.',
        suggestedImpact: null,
        impactJustification:
          'AI tidak dapat memberikan saran level dampak berdasarkan informasi yang diberikan.',
      }
    }

    const validatedLikelihood =
      output.suggestedLikelihood &&
      LIKELIHOOD_LEVELS_DESC.includes(
        output.suggestedLikelihood as LikelihoodLevelDesc
      )
        ? (output.suggestedLikelihood as LikelihoodLevelDesc)
        : null
    const validatedImpact =
      output.suggestedImpact &&
      IMPACT_LEVELS_DESC.includes(output.suggestedImpact as ImpactLevelDesc)
        ? (output.suggestedImpact as ImpactLevelDesc)
        : null

    return {
      suggestedLikelihood: validatedLikelihood,
      likelihoodJustification:
        output.likelihoodJustification ||
        'Tidak ada justifikasi tambahan dari AI.',
      suggestedImpact: validatedImpact,
      impactJustification:
        output.impactJustification || 'Tidak ada justifikasi tambahan dari AI.',
    }
  }
)
