...
const SuggestKriToleranceInputSchema = z.object({
  riskCauseDescription: z
    .string()
    .describe(
      'Deskripsi penyebab risiko yang akan dianalisis KRI dan Toleransinya.'
    ),
  ...
  riskCategory: z
    ...
    .nullable()
    .describe(
      `Kategori potensi risiko induk (opsional, untuk konteks tambahan). Kategori yang tersedia: ${RISK_CATEGORIES.join(
        ', '
      )}`
    ),
  goalDescription: z
    .string()
    .describe(
      'Deskripsi sasaran terkait untuk memberikan konteks keseluruhan.'
    ),
})
export type SuggestKriToleranceInput = z.infer<
  typeof SuggestKriToleranceInputSchema
>

const SuggestKriToleranceOutputSchema = z.object({
  suggestedKRI: z
    .string()
    .describe('Saran Key Risk Indicator (KRI) dalam Bahasa Indonesia.'),
  kriJustification: z
    .string()
    .describe(
      'Justifikasi atau alasan mengapa KRI tersebut disarankan, dalam Bahasa Indonesia.'
    ),
  suggestedTolerance: z
    .string()
    .describe(
      'Saran Toleransi Risiko untuk KRI yang disarankan, dalam Bahasa Indonesia.'
    ),
  toleranceJustification: z
    .string()
    .describe(
      'Justifikasi atau alasan mengapa Toleransi Risiko tersebut disarankan, dalam Bahasa Indonesia.'
    ),
})
export type SuggestKriToleranceOutput = z.infer<
  typeof SuggestKriToleranceOutputSchema
>
...
const suggestKriToleranceFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      console.warn(
        '[suggestKriToleranceFlow] AI output for KRI/Tolerance suggestion was null or undefined.'
      )
      return {
        suggestedKRI: 'AI tidak dapat memberikan saran KRI saat ini.',
        kriJustification: 'Tidak ada justifikasi dari AI.',
        suggestedTolerance:
          'AI tidak dapat memberikan saran Toleransi Risiko saat ini.',
    ...
    return {
      ...
      kriJustification:
        output.kriJustification || 'Tidak ada justifikasi KRI dari AI.',
      suggestedTolerance:
        output.suggestedTolerance || 'Tidak ada saran Toleransi dari AI.',
      toleranceJustification:
        output.toleranceJustification ||
        'Tidak ada justifikasi Toleransi dari AI.',
    }
  }
)
