...
const BrainstormRiskCausesInputSchema = z.object({
  ...
  potentialRiskCategory: z
    .custom<RiskCategory>()
    .nullable()
    .describe(
      'Kategori dari potensi risiko (opsional, untuk konteks tambahan).'
    ),
  goalDescription: z
    .string()
    .describe(
...
export type BrainstormRiskCausesInput = z.infer<
  typeof BrainstormRiskCausesInputSchema
>

const RiskCauseSuggestionSchema = z.object({
  description: z
    .string()
    .describe('Deskripsi singkat dari saran penyebab risiko.'),
  source: z
    .custom<RiskSource>()
    .nullable()
    .describe(
      'Saran sumber penyebab risiko (Internal atau Eksternal), atau null jika tidak dapat ditentukan dengan pasti.'
    ),
})
...
const BrainstormRiskCausesOutputSchema = z.object({
  suggestedCauses: z
    ...
    .describe(
      'Daftar saran penyebab risiko, masing-masing dengan deskripsi dan sumbernya.'
    ),
})
export type BrainstormRiskCausesOutput = z.infer<
  typeof BrainstormRiskCausesOutputSchema
>
...
${
  input.potentialRiskCategory
    ? `- Kategori Potensi Risiko: "${input.potentialRiskCategory}" (Kategori yang tersedia: ${availableRiskCategories})`
    : ''
}
  ...
  async (input) => {
    ...
    if (output && Array.isArray(output.suggestedCauses)) {
      ...
      const validatedCauses = output.suggestedCauses.map((cause) => ({
        ...cause,
        source:
          cause.source && RISK_SOURCES.includes(cause.source as RiskSource)
            ? cause.source
            : null,
      }))
    ...
    console.warn(
      'AI output for risk cause suggestions was not in the expected format.'
    )
