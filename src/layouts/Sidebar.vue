  ...
  <aside class="w-64 bg-card border-r border-border flex-col hidden md:flex">
    ...
    <nav class="flex-1 p-2 space-y-1">
      ...
      >
        <Button
          :variant="
            isActiveRoute(item.href, $route.path) ? 'secondary' : 'ghost'
          "
          class="w-full justify-start"
          :disabled="
            item.disabled && item.disabled(authStore.isProfileComplete)
          "
          @click="navigate"
        >
...
import { ref } from 'vue'
import {
  useRoute, // useRoute untuk mendapatkan path saat ini
} from 'vue-router'
...
const navItems = ref([
  {
    ...
    disabled: (isProfileComplete) => !isProfileComplete,
  },
  {
    label: 'Sasaran',
    href: '/goals',
